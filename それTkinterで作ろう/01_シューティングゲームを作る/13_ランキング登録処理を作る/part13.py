...
class GameWindow:
    ...
    def create_ranking_window(self):
        ...
        self.ranking.write(score)
        for i, rank in enumerate(self.ranking.get_rank()):
            label_date = tkinter.Label(self.rank_frame, text=rank[0], font="メイリオ, 12")
            label_date.grid(column=0, row=i+1, pady=5)
            label_score = tkinter.Label(self.rank_frame, text=rank[1], font="メイリオ, 12")
